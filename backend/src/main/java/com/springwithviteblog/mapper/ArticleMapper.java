...
public interface ArticleMapper {
  @Insert("INSERT INTO articles (author_id, title, content_type, content_raw, content_html, summary, cover_url, status, visibility, rss_enabled, allow_index, created_at, updated_at, published_at) "
      + "VALUES (#{authorId}, #{title}, #{contentType}, #{contentRaw}, #{contentHtml}, #{summary}, #{coverUrl}, #{status}, #{visibility}, #{rssEnabled}, #{allowIndex}, #{createdAt}, #{updatedAt}, #{publishedAt})")
  @Options(useGeneratedKeys = true, keyProperty = "id")
  int insert(Article article);

  @Update("UPDATE articles SET title = #{title}, content_type = #{contentType}, content_raw = #{contentRaw}, content_html = #{contentHtml}, summary = #{summary}, cover_url = #{coverUrl}, status = #{status}, visibility = #{visibility}, rss_enabled = #{rssEnabled}, allow_index = #{allowIndex}, updated_at = #{updatedAt}, published_at = #{publishedAt} WHERE id = #{id}")
